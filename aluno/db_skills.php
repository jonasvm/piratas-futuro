...
$sql = mysqli_query('SELECT p.xp,p.hp,p.max_hp,p.energy,p.max_energy FROM characterr as p, user as u WHERE p.user_id = u.id AND u.id = '.$id.'') or die(mysqli_error($con));
$row = mysqli_fetch_row($sql);

/*        _ Fazer update de energia/vida do usuário _        */
$sql = mysqli_query("SELECT l_login_data FROM characterr WHERE user_id = '$id' ") or die(mysqli_error($con));
...
if( ( $row[1] < $row[2] ) || ( $row[3] < $row[4] ) ) {
	mysqli_query(" UPDATE characterr SET l_login_data = '$date' WHERE user_id = '$id' ") or die(mysqli_error($con));
	
	//Se a vida dele for menor que a maxima
	if ( $row[1] < $row[2] ) {
		if( ($row[1] + $e_l) < $row[2] ) {
			mysqli_query("UPDATE characterr SET hp = hp + " . $e_l . " WHERE user_id = '$id'") or die(mysqli_error($con));
		} else {
			mysqli_query("UPDATE characterr SET hp = " . $row[2] . " WHERE user_id = '$id'") or die(mysqli_error($con));
		}
	}
	...
	if ( $row[3] < $row[4] ) {
		if( ($row[3] + $e_l) < $row[4] ){
			mysqli_query("UPDATE characterr SET energy = energy + " . $e_l . " WHERE user_id = '$id'") or die(mysqli_error($con));
		} else {
			mysqli_query("UPDATE characterr SET energy = " . $row[4] . " WHERE user_id = '$id'") or die(mysqli_error($con));
		}
	}

} else {
	mysqli_query(" UPDATE characterr SET l_login_data = '$cur_date' WHERE user_id = '$id' ") or die(mysqli_error($con));
}
		...
		<div id="main">
                    ...
                    <div id="user_picture_bg">
                    	<div class="user_picture">
							<?php 
                                ...
                                $query = mysqli_query("SELECT ext, data FROM user WHERE id='$id' ") or die(mysqli_error($con));
			...
			<div id="content">
				<div id="text">
						<?php
						include "conecta_mysql.inc";
						$sql = mysqli_query("SELECT s.name,s.description FROM skill AS s, character_skill AS cs WHERE cs.skill_id = s.id AND cs.user_id = " . $_SESSION["id"]) or die(mysqli_error($con));
						$row = mysqli_fetch_array($sql);
						$sql2 = mysqli_query("SELECT * FROM skill AS s, character_skill AS cs WHERE cs.skill_id = s.id AND cs.user_id = " . $_SESSION["id"]);
						$rows_skill = mysqli_num_rows($sql2);
						$html = '';
						$html .= '<table border="0" width="600">';
						$html .= '<tr>';
						$html .= '<h2>Habilidades</h2></td></tr>';
						$html .= '<tr><td align="center"><b>Nome</b></td>';
						$html .= '<td align="center"><b>Descrição</b></td></tr>';
						for ($i = 1; $i <= $rows_skill; $i++) {
							$sql = mysqli_query("SELECT s.name,s.description FROM skill AS s, character_skill AS cs WHERE cs.skill_id = s.id AND cs.user_id = " . $_SESSION["id"]) or die(mysqli_error($con));
							$row = mysqli_fetch_array($sql);
