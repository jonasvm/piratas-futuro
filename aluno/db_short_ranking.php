    ...
    <body>
		<div id="main">
			...
			<div id="content">
				...
				include "conecta_mysql.inc";
				$html = '';
				$html .= '<center><table border="0" width="900">';
				$html .= '<tr class="atr"><td align="center"><b>Colocação</b></td>';
				$html .= '<td align="center"><b>Imagem</b></td>';
				$html .= '<td align="center"><b>Nome do jogador</b></td>';
				...
				while ($linha = mysqli_fetch_array($sql2)) {
					$i++;
					$query = mysqli_query($con, 'SELECT ext, data FROM user WHERE id = '.$linha['uid']) or die(mysqli_error($con));
					
					while ($imagens = mysqli_fetch_array($query)){
						$dados['ext'] = $imagens['ext'];
						$dados['data'] = $imagens['data'];
					}
					$html .= '<tr class="atrV" ><td width="10%">' . $i . 'º</td><td width="80">' .
								'<img src="data:' . $dados['ext'] . ';base64,' . base64_encode( $dados['data'] ) . '" width="80" height="80" /></td><td>' .
								$linha['uname'] . '</td><td>' . $linha['cname'] . '</td><td width="10%">' . $linha['xp'] . '</td></tr>';
				}
