...
$sql = mysqli_query($con, 'SELECT p.xp,p.hp,p.max_hp,p.energy,p.max_energy,p.lvl,p.location FROM characterr as p, user as u WHERE p.user_id = u.id AND u.id = '.$id.'') or die(mysqli_error($con));
$row = mysqli_fetch_row($sql);
if($row[6] != 5){
	mysqli_query($con, "UPDATE characterr SET location = '5' WHERE user_id = '$id'") or die(mysqli_error($con));
}
?>
    ...
    <body>
		<div id="mainOverlay">
			<div id="allOpacity">
				...
				$query_c = mysqli_query($con, "SELECT location FROM characterr WHERE user_id='$id' ") or die(mysqli_error($con));
				$loc = mysqli_fetch_array($query_c);
				...
				if($loc[0] != 5){
					mysqli_query($con, "UPDATE user SET location='5' WHERE id ='$id'");
				}
			...
			<div id="map">
				<a href="map.php"><img class="pointer" src="images/seta.png" width="50" height="50" title="Voltar ao Mapa Geral"/></a>
				<a href="index.php"><img class="cancel" src="images/cancel.png" width="20" height="20" title="Sair"/></a>
				<img src="images/Ilha dos Rebeldes.png" alt="" height="580" usemap="#Map" border="0" />
				<?php if( $row[5] == 5 ) { ?>
					<a href="db_change_map.php?location_id=5.php"><img class="pin" src="images/pin.png" width="30" height="30" title="Mude sua localização para a Ilha dos Rebeldes"/></a>
				<?php } ?>
			</div>
		</div> 
	</body>
