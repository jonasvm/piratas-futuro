...
mysqli_query($con, "INSERT into activity_student (name, activity, data, hour) VALUES ('$row[5]', 'Ranking', curdate(), curtime())");
?>
    ...
    <body>
		<div id="main">
			...
			<div id="text">
				<?php
				include "conecta_mysql.inc";
				//função q faz as cores do ranking
				...
				$html = '';
				$html .= '<center><table class="tableRanking" border="0" width="900">';
				$html .= '<tr class="atr"><th align="center"><b>Colocação</b></th>';
				$html .= '<th align="center"><b>Imagem</b></th>';
				$html .= '<th align="center"><b>Nome do jogador</b></th>';
				$html .= '<th align="center"><b>Nome do personagem</b></th>';
				$html .= '<th align="center"><b>XP</b></th></tr>';
				
				$sql2 = mysqli_query($con, "SELECT c.name as cname, c.xp as xp, u.name as uname, u.id as uid FROM characterr as c, user as u WHERE u.id = c.user_id AND u.type = 3 ORDER BY c.xp DESC") or die(mysqli_error($con));
				$i = 0;
				
				while ($linha = mysqli_fetch_array($sql2)) {
					$i++;
					$query = mysqli_query($con, 'SELECT ext, data, upload FROM user WHERE id = '.$linha['uid']) or die(mysqli_error($con));
					...
					if($dados['upload'] == 0)
						$html .= '<tr class="atrV"><td width="10%">' . $i . '</td>
						  	  <td width="80">' . '<img src="data:image/' . $dados['ext'] . ';base64,' . base64_encode( $dados['data'] ) . '" width="80" height="80" /></a>
					          	  </td><td>' . $linha['uname'] . '</td><td>' . $linha['cname'] . '</td><td width="10%">' . $linha['xp'] . '</td></tr>';
					else
						$html .= '<tr class="atrV" ><td width="10%">' . $i . '</td>
						  	  <td width="80">' . '<img src="fotos_perfil/'.$dados['data']. '" width="80" height="80" />
					          	  </td><td>' . $linha['uname'] . '</td><td>' . $linha['cname'] . '</td><td width="10%">' . $linha['xp'] . '</td></tr></div>';
				}
