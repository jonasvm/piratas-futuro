...
/*        _ Fazer update de energia/vida do usuÃ¡rio _        */
...
$last_l = strtotime($lastLogin[0]); //data no banco de dados em formato 'numÃ©rico'
$c_hour = strtotime( $cur_date ); //data atual no formato 'numÃ©rico'

$e_l = floor((($c_hour - $last_l)/60)/10); //quantidade de vida/energia a aumentar

$total = (($c_hour - $last_l)/60); //diferenÃ§a entre a data atual e a data do banco de dados (min + seg)
$min = ($total - (10*$e_l)); //apenas os minutos decorridos desde a ultima atualizaÃ§Ã£o (formato 'numÃ©rico')
$dif = $min*60; //diferenÃ§a em segundos
$date = $c_hour - $dif; //horÃ¡rio com o resto
$date = date("d-m-Y H:i:s",$date); //data com o resto da ultima atualizaÃ§Ã£o
//Se vida ou energia ainda nao estao no mÃ¡ximo, a hora no BD Ã© atualizada contando o 'resto' da hora anterior
if( ( $row[1] < $row[2] ) || ( $row[3] < $row[4] ) ) {
	...
	//Se a energia dele for menor que a mÃ¡xima e a diferenÃ§a
	if ( $row[3] < $row[4] ) {
		if( ($row[3] + $e_l) < $row[4] ){
		...
		<div id="main">
			...
			<div id="content">
				<div id="text">
					...
					<p>Tudo que um pirata poderia desejar pelo menor preço do mercado!</p>
					<br><a href="db_store.php"><p>Comprar</a> | <a href="db_store_sell.php">Vender</p></a><br>
						...
						$html .= '<li align="center"><a href="db_store.php">Armas Brancas</a></li>';
						$html .= '<li align="center"><a href="armas_de_fogo.php">Armas de Fogo</a></li>';
						$html .= '<li align="center"><a href="armaduras.php">Armaduras</a></li>';
						$html .= '<li class="selected" align="center"><a href="consumiveis.php">Consumíveis</a></li>';
						$html .= '</ul>';
						$html .= '<table class="tableStore"  border="0" width="1200">';
						$html .= '<tr class="atr"><td><p class="title">Imagem</p></td><td><p class="title">Item</p></td>';
						$html .= '<td align="center"><p class="title">Informações</p></td><td><p class="title">Level Requerido</p></td><td><p class="title">Preço</p></td></tr>';
						$sql = mysqli_query($con, "SELECT level_required,name,description,id,buy_value,slot FROM item WHERE type='4' ORDER BY level_required") or die(mysqli_error($con));
						...
						while ($row2 = mysqli_fetch_assoc($sql)) {
							...
							if($dados['upload'] == 0)
								$html .= '<tr class="atrV"><td><center><img src="data:image/' . $dados['ext'] . ';base64,' . base64_encode( $dados['data'] ) . '" width="92" height="102"/>' . '</td><td><center>' . utf8_encode($row2['name']) . '</td><td><center>Equipado: ' . $slot . '<br>' . utf8_encode($row2['description']) . '</td><td><center>' . $row2['level_required'] . '</td><td><form id="form1" method="post" action="db_buy.php"><center>' . $row2['buy_value'] . ' Battle Points <br />';
							else
								$html .= '<tr class="atrV"><td><center><img src="../administrador/fotos_armas/'.$dados['data']. '" width="92" height="102"/>' . '</td><td><center>' . utf8_encode($row2['name']) . '</td><td><center>Equipado: ' . $slot . '<br>' . utf8_encode($row2['description']) . '</td><td><center>' . $row2['level_required'] . '</td><td><form id="form1" method="post" action="db_buy.php"><center>' . $row2['buy_value'] . ' Battle Points <br />';

							if( $row[5] < $row2['level_required'] )
								$html .= '<input type="submit" value="Comprar" disabled="disabled" /></td>';
							else 
								$html .= '<input type="submit" value="Comprar" /></td>';
								
							$html .= '<input name="user_id" id="user_id" type="hidden" value="' . $_SESSION["id"] . '" /><input name="item_id" id="item_id" type="hidden" value="' . $row2['id'] . '" /></form></tr>';
						}
						$html .= '</table>';
						echo $html;
						?>
