...
mysqli_query($con, "INSERT into activity_student (name, activity, sub_activity, result, data, hour) VALUES ('$row[5]', 'Desafio','','', curdate(), curtime())") or die(mysqli_error($con));
		...
		<div id="main">
			...
			<div id="content">
				<div id="text">
						...
						$sql = mysqli_query($con, 'SELECT id,title,level,energy FROM challenge WHERE level='.$row_location[0]) or die(mysqli_error($con));
						$rows_challenges = mysqli_affected_rows($con);
						...
						<form><?php
							...
							$html .= '<tr>';
							$html .= '<td colspan="5"><p class="h2">Lista de problemas</p></td></tr>';
							...
							$sql1 = mysqli_query($con, 'SELECT id,title,level,energy FROM challenge') or die(mysqli_error($con));
							$i = 0;
							while ($row1 = mysqli_fetch_assoc($sql1)) {
								if($row1['level'] == $row_location[0]){
									$lista[$i][0] = $row1['id'];
									$lista[$i][1] = utf8_encode($row1['title']);
							...
							for ($j = 0; $j <= $i-1; $j++) {
								$html .= '<tr class="atrV"><td><a href="db_detail_challenge.php?mission_id=' . $lista[$j][0] . '">' . ($j+1) . '</a></td><td><a href="db_detail_challenge.php?mission_id=' . $lista[$j][0] . '">' . $lista[$j][1] . '</a></td><td><a href="db_detail_challenge.php?mission_id=' . $lista[$j][0] . '">' . $lista[$j][2] . '</a></td><td><a href="db_detail_challenge.php?mission_id=' . $lista[$j][0] . '">' . $lista[$j][3] . '</a></td></tr>';
							}
