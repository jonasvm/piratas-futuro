...
//Dados do usuário
$sql = mysqli_query($con, 'SELECT p.xp, p.hp, p.max_hp, p.energy, p.max_energy, p.name FROM characterr as p, user as u WHERE p.user_id = u.id AND u.id = '.$user_id.'') or die(mysqli_error($con));
$row = mysqli_fetch_row($sql);

//Insere atividade do aluno
mysqli_query($con, "INSERT into activity_student (name, activity, sub_activity, result, data, hour) VALUES ('$row[5]', 'Missão','','', curdate(), curtime())") or die(mysqli_error($con));
?>
		...
		<div id="main">
			...
			<div id="content">
				<div id="text">
								<?php
								include "conecta_mysql.inc";
								$sql = mysqli_query($con, 'SELECT location FROM characterr WHERE user_id='.$user_id) or die(mysqli_error($con));
								$row_location = mysqli_fetch_array($sql);
								
								$sql = mysqli_query($con, 'SELECT id,name,level,energy FROM mission WHERE level = '.$row_location[0]) or die(mysqli_error($con));
								$rows_mission = mysqli_affected_rows($con);
								...
								<form><?php
									...
									$html .= '<tr>';
									$html .= '<td colspan="5"><p class="h2">Lista de missões</p></td></tr>';
									$html .= '<tr class="atr"><td><p class="title">Id</p></td>';
									$html .= '<td><p class="title">Título</p></td><td><p class="title">Nível</p></td><td><p class="title">Energia</p></td></tr>';
									
									
									
									$sql1 = mysqli_query($con, 'SELECT id,name,level,energy FROM mission') or die(mysqli_error($con));
									$i = 0;
									while ($row1 = mysqli_fetch_assoc($sql1)) {
										if($row1['level'] == $row_location[0]){
											$lista[$i][0] = $row1['id'];
											$lista[$i][1] = utf8_encode($row1['name']);
											$lista[$i][2] = $row1['level'];
											$lista[$i][3] = $row1['energy'];
											$i++;
										}
									}
									
									for ($j = 0; $j <= $i-1; $j++) {
										$html .= '<tr class="atrV"><td><a href="db_detail_mission.php?id=' . $lista[$j][0] . '">' . ($j+1) . '</a></td><td><a href="db_detail_mission.php?id=' . $lista[$j][0] . '">' . $lista[$j][1] . '</a></td><td><a href="db_detail_mission.php?id=' . $lista[$j][0] . '">' . $lista[$j][2] . '</a></td><td><a href="db_detail_mission.php?id=' . $lista[$j][0] . '">' . $lista[$j][3] . '</a></td></tr>';
									}
