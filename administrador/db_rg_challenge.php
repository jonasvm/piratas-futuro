...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div id="main">
			...
			<div id="content">
				<h2 class="title">Cadastro de Desafios</h2>
				<form accept-charset="latin1" enctype="multipart/form-data" name="form" method="post" action="register_challenge.php" onSubmit="return validate(this)" >
					<table align="center">
						...
						<td class="col2" width="35%">
							...
							<p>
								9 - Requerimentos:<br>
							<p><input type="checkbox" name="requeriments[]" id="req_chal" value="chal" onclick="habilitar_chal();" /> Desafio(s) necess&aacute;rio(s)
							...
							<br><select id="label_req_chal" name="label_req_chal" size="5" disabled="disabled" >
								...
								while ($row = mysqli_fetch_assoc($sql)) {
									$html .= '<option value="'.utf8_encode($row['title']).'">'.utf8_encode($row['title']).'</option>';
								}
							...
							<br><select id="label_req_equip" name="label_req_equip" size="7" disabled="disabled" >
								...
								while ($row = mysqli_fetch_assoc($sql)) {
									$html .= '<option value="'.utf8_encode($row['name']).'">'.utf8_encode($row['name']).'</option>';
								}
