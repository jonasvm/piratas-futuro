...
if(isset($_GET["chal"]))
{
	...
	$deleteChal2 = mysqli_query ($con, "DELETE FROM armas_de_fogo WHERE name = '$rowName[0]'") or die(mysqli_error($con));
	...
	<head>
		...
		function Deletar(titulo, id)
		{
			if (confirm("Deseja deletar \n" + titulo + " ?")) {
				window.location = 'armas_de_fogo.php?chal=' + id;
			}
		}
		</script>
	...
	<body>
		<div id="main">
			...
			<div id="content">
				...
				$html .= '<li class="selected"><a href="armas_de_fogo.php">Armas de Fogo</a></li>';
				...
				$sql = mysqli_query($con, "SELECT level_required,name,description,id,buy_value,slot,sale_value FROM item WHERE type='1' ORDER BY level_required") or die(mysqli_error($con));
				...
				while ($row2 = mysqli_fetch_assoc($sql))
				{
					...
					while ($imagens = mysqli_fetch_array($sql2)){
						...
						$html .= '<center>Venda: ' . $row2['sale_value'] . ' moedas pirata <br /></td><td><a href="db_edit_item.php?item_id=' . $row2['id'] . '&item_type=1">';
